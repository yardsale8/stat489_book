Sequential Data Structures in Python
::::::::::::::::::::::::::::::::::::

.. toctree::
    :caption: Sequential Data Structures in Python
    :maxdepth: 1

    CollectionTypes_StringsListsTuples.rst
    OperationsOnSequences.rst
    AccessingSequenceData.rst
    AskingBooleanQuestionsAboutSequences.rst
    MutableAndImmutableSequences.rst
    ReferencesToSequences.rst
    SequenceMethodsandWorkingwithStringsAndLists.rst
    ComputationalComplexityAndBigONotation.rst
    TransformingSequencesWithComprehensions.rst
    OtherPythonSequenceTypes.rst
    Exercises.rst
